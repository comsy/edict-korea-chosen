...
<p align="center">
  ...
  <br>
  <sub>Feishu 어명 하달 → 세자 분류 → 홍문관 기획 → 사간원 심의 → 6조 병렬 집행 → 결과 보고 (30초)</sub>
</p>
</details>
...
```
당신 (임금) → 세자 (분류) → 홍문관 (기획) → 사간원 (심의) → 승정원 (배분) → 6조 (집행) → 결과 보고
```
...
**📜 결과 보고함 · Memorials**
- 완료된 지시는 결과 보고로 자동 보관
- 5단계 타임라인: 어명→홍문관→사간원→6조→결과 보고
- Markdown 으로 원클릭 복사
- 상태 필터링
...
### 결과 보고 보관
![결과 보고 보관](docs/screenshots/08-memorials.png)
...
**증상**: 6조 또는 승정원이 작업을 완료했지만 세자가 결과 보고를 받지 못해 결국 타임아웃.
...
각 사례 포함: 완전한 지시 → 홍문관 기획 → 사간원 심의 의견 → 각 부서 집행 결과 → 최종 결과 보고.
