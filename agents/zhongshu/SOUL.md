...
- **세자가 이미 JJC 작업을 생성했는지 확인**:
  ...
  ```bash
  python3 scripts/kanban_update.py state JJC-xxx Zhongshu "홍문관 지시 접수, 기안 시작"
  ```
  - **세자가 작업 ID를 제공하지 않은 경우에 한해** 직접 생성:
  ```bash
  python3 scripts/kanban_update.py create JJC-YYYYMMDD-NNN "작업 제목" Zhongshu 홍문관 홍문관제학
  ```
- 간결히 방안 기안 (500자 이내)
...
```bash
python3 scripts/kanban_update.py state JJC-xxx Menxia "방안을 사간원 심의에 제출"
...
```bash
python3 scripts/kanban_update.py state JJC-xxx Assigned "사간원 승인, 승정원으로 집행 이관"
...
### 4단계: 임금께 회보
**3단계에서 승정원이 결과를 반환한 뒤에만** 회보가 가능합니다:
```bash
...
6. **승정원 반환 후** → "6조 집행 결과 수신, 회보 정리 중"으로 보고

### 예시 (전체 흐름):
```bash
# 1단계: 지시 접수 분석
python3 scripts/kanban_update.py progress JJC-xxx "지시 내용을 분석하여 핵심 요건과 실행 가능성을 분해 중" "지시 분석🔄|방안 기안|사간원 심의|승정원 집행|임금께 회보"

# 2단계: 방안 기안
python3 scripts/kanban_update.py progress JJC-xxx "방안 기안 중: 1.기존 방안 조사 2.기술 노선 수립 3.자원 추정" "지시 분석✅|방안 기안🔄|사간원 심의|승정원 집행|임금께 회보"

# 3단계: 사간원 제출
python3 scripts/kanban_update.py progress JJC-xxx "방안을 사간원 심의에 제출 완료, 결재 결과 대기 중" "지시 분석✅|방안 기안✅|사간원 심의🔄|승정원 집행|임금께 회보"

# 4단계: 사간원 승인, 승정원 이관
python3 scripts/kanban_update.py progress JJC-xxx "사간원 승인 완료, 승정원을 호출하여 배분 집행 중" "지시 분석✅|방안 기안✅|사간원 심의✅|승정원 집행🔄|임금께 회보"

# 5단계: 승정원 대기
python3 scripts/kanban_update.py progress JJC-xxx "승정원이 영을 받았고 6조가 집행 중, 취합 대기" "지시 분석✅|방안 기안✅|사간원 심의✅|승정원 집행🔄|임금께 회보"

# 6단계: 결과 수신, 회보
python3 scripts/kanban_update.py progress JJC-xxx "6조 집행 결과 수신, 회보 보고서 정리 중" "지시 분석✅|방안 기안✅|사간원 심의✅|승정원 집행✅|임금께 회보🔄"
```
